...
+ Условие входа и выхода: в данной модели формулируется так: вход и выход располагаем достаточно
  далеко от областей с высокими градиентами скорости, поэтому считаем
  $(partial bold(v))/(partial bold(n)) |_(Sigma_1) = 0$.
...
Для условий входа и выхода условие преобразуем к виду:
$
(partial psi)/(partial bold(n)) |_(Sigma_1) = bold(n) dot nabla psi |_(Sigma_1) = 0.
$
...
следующим образом:
$
psi^((i)) = sum_(j=1)^N psi_j dot N_j (x, y), \
delta psi^((i)) = sum_(j=1)^N delta psi_j dot N_j (x, y)
$
#par(first-line-indent: 0pt)[
...
$
sum_(i in"КЭ") sum_(j in"узлы КЭ") sum_(k in"узлы КЭ") psi_j delta psi_k limits(integral.double)_(V_i) nabla N_j dot nabla N_k d V = 0,
$
#par(first-line-indent: 0pt)[
  меняя порядок суммирования, вынесем суммирование по $k$ наружу:
]
$
sum_(k in"все узлы") delta psi_k sum_(i in "КЭ, содержащие k") sum_(j in "узлы i-го КЭ") psi_j limits(integral.double)_(V_i) nabla N_j dot nabla N_k space d V = 0,
$
#par(first-line-indent: 0pt)[
  т.к. $delta psi_k$ -- произвольные,
  а уравнения должны выполняться для любых $delta psi_k$, из вариационного принципа, коэффициенты
  перед $delta psi_k$ должны быть равны нулю:
]
$
sum_(i in "КЭ, содержащие k") sum_(j in "узлы i-го КЭ") psi_j limits(integral.double)_(V_i) nabla N_j dot nabla N_k space d V = 0.
$

Обозначая ${psi} = vec(psi_1, psi_2, dots, psi_N)$, $[K] = (K_(i j))^(N times N)$:
$
K_(i j) = sum_(k in "КЭ, содержащие i") limits(integral.double)_(V_k) nabla N_i dot nabla N_j space d V_k.
$

== Линейные симплекс элементы

Для линейных симлекс элементов, функции формы удобно выбрать в новых координатах -- т.н.
L-координатах: $L_i$ принимает значение 1 в $i$-ом узле, и значение 0 на противоположной грани.

В таком элементе $N_i = L_i$.

Декартовы координаты связаны с L-координатами следующими соотношениями:
$
vec(x, y, z) = mat(x_1, x_2, x_3; y_1, y_2, y_3; 1, 1, 1) dot vec(L_1, L_2, L_3)
<=> \
<=>
vec(L_1, L_2, L_3) = 1/det(J)
mat(
  y_2 - y_3, -(x_2 - x_3), x_2 y_3 - x_3 y_2;
  - (y_1 - y_3), x_1 - x_3, - (x_1 y_3 - x_3 y_1);
  y_1 - y_2, - (x_1 - x_2), x_1 y_2 - x_2 y_1
) vec(x, y, 1)
$
$
det(J)  &= x_2 y_3 - x_3 y_2 - x_1 y_3 + x_3 y_1 + x_1 y_2 - x_2 y_1 = \
        &= x_1 (y_2 - y_3) - x_2 (y_1 - y_3) + x_3 (y_1 - y_2)
$
#par(first-line-indent: 0pt)[
  где $x_i, y_i$ -- координаты $i$-ой вершины треугольника.
]

В L-координатах легче считать следующие интегралы:
$
limits(integral.double)_(V_i) L_1^a L_2^b L_3^c space d V = 2 S (a! b! c!) / (a+b+c+2)! \
limits(integral)_(cal(L)_1) L_2^a L_3^b space d cal(L) = cal(L)_1 (a! b!) / (a+b+1)!,
$
#par(first-line-indent: 0pt)[
  где $S$ -- площадь треугольника;
  $cal(L)_1$ -- отрезок $L_1 = 0$, а также его длина.
]

Также посчитаем, чему равны градиенты $N_i$:
$
nabla N_i = ( (partial N_i)/(partial x^j) )_(i j) = 1/(det J) mat(
  y_2 - y_3, - (x_2 - x_3);
  -(y_1 - y_3), x_1 - x_3;
  y_1 - y_2, - (x_1 - x_2)
)
